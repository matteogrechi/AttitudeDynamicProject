...
#include "xdotc_SVcmKUHW.h"
#include "xnrm2_RrjzBGLz.h"
...
void svd_Fpugt20V(const real_T A[9], real_T U[9], real_T s[3], real_T V[9])
{
  real_T b_A[9];
  real_T b_s[3];
  real_T e[3];
  ...
  if (nrm > 0.0) {
    ...
    if (muDoubleScalarAbs(b_s[0]) >= 1.0020841800044864E-292) {
      xscal_7RF6om6K(3, 1.0 / b_s[0], b_A, 1);
    } else {
      for (qjj = 0; qjj < 3; qjj++) {
        b_A[qjj] /= b_s[0];
      }
  ...
  for (b_b = 1; b_b + 1 < 4; b_b++) {
    qjj = 3 * b_b;
    if (apply_transform) {
      xaxpy_KBGJ1XMW(3, -(xdotc_SVcmKUHW(3, b_A, 1, b_A, qjj + 1) / b_A[0]), 1,
                     b_A, qjj + 1);
    }
  ...
  if (nrm == 0.0) {
    ...
    for (qq = 1; qq + 1 < 4; qq++) {
      xaxpy_qZCI1Bpc(2, e[qq], b_A, 3 * qq + 2, work, 2);
    }

    for (qq = 1; qq + 1 < 4; qq++) {
      xaxpy_AMOJb3Go(2, -e[qq] / e[1], work, 2, b_A, 3 * qq + 2);
    }
  }
  ...
  if (nrm > 0.0) {
    ...
    if (muDoubleScalarAbs(b_s[1]) >= 1.0020841800044864E-292) {
      xscal_7RF6om6K(2, 1.0 / b_s[1], b_A, 5);
    } else {
      for (qjj = 4; qjj < 6; qjj++) {
        b_A[qjj] /= b_s[1];
      }
  ...
  for (b_b = 2; b_b + 1 < 4; b_b++) {
    qjj = 3 * b_b + 1;
    if (apply_transform) {
      xaxpy_KBGJ1XMW(2, -(xdotc_SVcmKUHW(2, b_A, 5, b_A, qjj + 1) / b_A[4]), 5,
                     b_A, qjj + 1);
    }
  ...
  for (b_b = 1; b_b >= 0; b_b--) {
    ...
    if (b_s[b_b] != 0.0) {
      for (kase = b_b + 1; kase + 1 < 4; kase++) {
        qjj = (3 * kase + b_b) + 1;
        xaxpy_KBGJ1XMW(3 - b_b, -(xdotc_SVcmKUHW(3 - b_b, U, qq + 1, U, qjj) /
          U[qq]), qq + 1, U, qjj);
      }
  ...
  for (qq = 2; qq >= 0; qq--) {
    if ((qq + 1 <= 1) && (e[0] != 0.0)) {
      for (b_b = 2; b_b < 4; b_b++) {
        qjj = (b_b - 1) * 3 + 2;
        xaxpy_KBGJ1XMW(2, -(xdotc_SVcmKUHW(2, V, 2, V, qjj) / V[1]), 2, V, qjj);
      }
    }
