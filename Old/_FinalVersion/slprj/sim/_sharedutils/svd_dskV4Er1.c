...
#include "xnrm2_m32bCKwj.h"
#include "xrot_HZ2sz8LY.h"
...
void svd_dskV4Er1(const real_T A[4], real_T U[4], real_T s[2], real_T V[4])
{
  real_T b_A[4];
  real_T b_s[2];
  real_T e[2];
  ...
  if (nrm > 0.0) {
    ...
    b_A[0]++;
    b_s[0] = -nrm;
    xaxpy_tPXkHMJq(-(xdotc_u6u7RC2O(b_A, b_A) / b_A[0]), b_A);
  } else {
    b_s[0] = 0.0;
  }
  ...
  U[3] = 1.0;
  if (b_s[0] != 0.0) {
    xaxpy_tPXkHMJq(-(xdotc_u6u7RC2O(U, U) / b_A[0]), U);
    U[0] = -U[0];
    U[1] = -U[1];
    U[0]++;
  } else {
  ...
  while ((m + 2 > 0) && (iter < 75)) {
    ...
    switch (kase) {
     case 1:
      ...
      qs = m + 1;
      while (qs >= q + 1) {
        xrotg_Aek4oMo2(&b_s[0], &rt, &ztest, &sqds);
        xrot_HZ2sz8LY(V, 1, ((m + 1) << 1) + 1, ztest, sqds);
        qs = 0;
      }
      break;
     ...
     case 2:
      ...
      e[q - 1] = 0.0;
      for (qs = q; qs < m + 2; qs++) {
        xrotg_Aek4oMo2(&b_s[qs], &rt, &ztest, &sqds);
        rt = -sqds * e[qs];
        e[qs] *= ztest;
        xrot_HZ2sz8LY(U, (qs << 1) + 1, ((q - 1) << 1) + 1, ztest, sqds);
      }
      break;
     ...
     default:
      ...
      while ((q + 1 < 2) && (b_s[q] < b_s[iter])) {
        ...
        b_s[q] = b_s[iter];
        b_s[iter] = rt;
        xswap_By3IoyvE(V, (q << 1) + 1, ((q + 1) << 1) + 1);
        xswap_By3IoyvE(U, (q << 1) + 1, ((q + 1) << 1) + 1);
        q = iter;
        iter++;
      }
