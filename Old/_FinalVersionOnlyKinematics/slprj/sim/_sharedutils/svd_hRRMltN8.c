#include "rtwtypes.h"
#include "multiword_types.h"
#include "mwmathutil.h"
#include "sqrt_tEibmDbk.h"
#include "xaxpy_WrDZ0dfu.h"
#include "xaxpy_a0PlI2Pz.h"
#include "xaxpy_xNufAhLU.h"
#include "xdotc_2UIAyAl6.h"
#include "xnrm2_PEgY4c29.h"
#include "xnrm2_XRtcWIHn.h"
#include "xrot_8SOrvP1x.h"
#include "xrotg_Aek4oMo2.h"
#include "xscal_LFP3qda1.h"
#include "xscal_j6B0CTAZ.h"
#include "xscal_xJbqLS2l.h"
#include "xswap_L2OVFn32.h"
#include "svd_hRRMltN8.h"

void svd_hRRMltN8(const real_T A[16], real_T U[16], real_T s[4], real_T V[16])
{
  real_T b_A[16];
  real_T b_s[4];
  real_T e[4];
  real_T work[4];
  real_T emm1;
  real_T nrm;
  real_T rt;
  real_T shift;
  real_T smm1;
  real_T sqds;
  real_T ztest;
  int32_T exitg1;
  int32_T k_ii;
  int32_T m;
  int32_T qjj;
  int32_T qp1jj;
  int32_T qp1q;
  int32_T qq;
  boolean_T apply_transform;
  boolean_T exitg2;
  e[0] = 0.0;
  work[0] = 0.0;
  e[1] = 0.0;
  work[1] = 0.0;
  e[2] = 0.0;
  work[2] = 0.0;
  e[3] = 0.0;
  work[3] = 0.0;
  for (qq = 0; qq < 16; qq++) {
    b_A[qq] = A[qq];
    U[qq] = 0.0;
    V[qq] = 0.0;
  }

  qq = 0;
  apply_transform = false;
  nrm = xnrm2_PEgY4c29(4, b_A, 1);
  if (nrm > 0.0) {
    apply_transform = true;
    if (b_A[0] < 0.0) {
      b_s[0] = -nrm;
    } else {
      b_s[0] = nrm;
    }

    if (muDoubleScalarAbs(b_s[0]) >= 1.0020841800044864E-292) {
      xscal_j6B0CTAZ(4, 1.0 / b_s[0], b_A, 1);
    } else {
      for (qjj = 0; qjj < 4; qjj++) {
        b_A[qjj] /= b_s[0];
      }
    }

    b_A[0]++;
    b_s[0] = -b_s[0];
  } else {
    b_s[0] = 0.0;
  }

  for (qp1q = 1; qp1q + 1 < 5; qp1q++) {
    qjj = qp1q << 2;
    if (apply_transform) {
      xaxpy_xNufAhLU(4, -(xdotc_2UIAyAl6(4, b_A, 1, b_A, qjj + 1) / b_A[0]), 1,
                     b_A, qjj + 1);
    }

    e[qp1q] = b_A[qjj];
  }

  while (qq + 1 < 5) {
    U[qq] = b_A[qq];
    qq++;
  }

  nrm = xnrm2_XRtcWIHn(3, e, 2);
  if (nrm == 0.0) {
    e[0] = 0.0;
  } else {
    if (e[1] < 0.0) {
      e[0] = -nrm;
    } else {
      e[0] = nrm;
    }

    nrm = e[0];
    if (muDoubleScalarAbs(e[0]) >= 1.0020841800044864E-292) {
      xscal_LFP3qda1(3, 1.0 / e[0], e, 2);
    } else {
      for (qq = 1; qq < 4; qq++) {
        e[qq] /= nrm;
      }
    }

    e[1]++;
    e[0] = -e[0];
    for (qq = 1; qq + 1 < 5; qq++) {
      work[qq] = 0.0;
    }

    for (qq = 1; qq + 1 < 5; qq++) {
      xaxpy_WrDZ0dfu(3, e[qq], b_A, (qq << 2) + 2, work, 2);
    }

    for (qq = 1; qq + 1 < 5; qq++) {
      xaxpy_a0PlI2Pz(3, -e[qq] / e[1], work, 2, b_A, (qq << 2) + 2);
    }
  }

  for (qq = 1; qq + 1 < 5; qq++) {
    V[qq] = e[qq];
  }

  apply_transform = false;
  nrm = xnrm2_PEgY4c29(3, b_A, 6);
  if (nrm > 0.0) {
    apply_transform = true;
    if (b_A[5] < 0.0) {
      b_s[1] = -nrm;
    } else {
      b_s[1] = nrm;
    }

    if (muDoubleScalarAbs(b_s[1]) >= 1.0020841800044864E-292) {
      xscal_j6B0CTAZ(3, 1.0 / b_s[1], b_A, 6);
    } else {
      for (qjj = 5; qjj < 8; qjj++) {
        b_A[qjj] /= b_s[1];
      }
    }

    b_A[5]++;
    b_s[1] = -b_s[1];
  } else {
    b_s[1] = 0.0;
  }

  for (qp1q = 2; qp1q + 1 < 5; qp1q++) {
    qjj = (qp1q << 2) + 1;
    if (apply_transform) {
      xaxpy_xNufAhLU(3, -(xdotc_2UIAyAl6(3, b_A, 6, b_A, qjj + 1) / b_A[5]), 6,
                     b_A, qjj + 1);
    }

    e[qp1q] = b_A[qjj];
  }

  for (qq = 1; qq + 1 < 5; qq++) {
    U[qq + 4] = b_A[qq + 4];
  }

  nrm = xnrm2_XRtcWIHn(2, e, 3);
  if (nrm == 0.0) {
    e[1] = 0.0;
  } else {
    if (e[2] < 0.0) {
      e[1] = -nrm;
    } else {
      e[1] = nrm;
    }

    nrm = e[1];
    if (muDoubleScalarAbs(e[1]) >= 1.0020841800044864E-292) {
      xscal_LFP3qda1(2, 1.0 / e[1], e, 3);
    } else {
      for (qq = 2; qq < 4; qq++) {
        e[qq] /= nrm;
      }
    }

    e[2]++;
    e[1] = -e[1];
    for (qq = 2; qq + 1 < 5; qq++) {
      work[qq] = 0.0;
    }

    for (qq = 2; qq + 1 < 5; qq++) {
      xaxpy_WrDZ0dfu(2, e[qq], b_A, (qq << 2) + 3, work, 3);
    }

    for (qq = 2; qq + 1 < 5; qq++) {
      xaxpy_a0PlI2Pz(2, -e[qq] / e[2], work, 3, b_A, (qq << 2) + 3);
    }
  }

  for (qq = 2; qq + 1 < 5; qq++) {
    V[qq + 4] = e[qq];
  }

  apply_transform = false;
  nrm = xnrm2_PEgY4c29(2, b_A, 11);
  if (nrm > 0.0) {
    apply_transform = true;
    if (b_A[10] < 0.0) {
      b_s[2] = -nrm;
    } else {
      b_s[2] = nrm;
    }

    if (muDoubleScalarAbs(b_s[2]) >= 1.0020841800044864E-292) {
      xscal_j6B0CTAZ(2, 1.0 / b_s[2], b_A, 11);
    } else {
      for (qjj = 10; qjj < 12; qjj++) {
        b_A[qjj] /= b_s[2];
      }
    }

    b_A[10]++;
    b_s[2] = -b_s[2];
  } else {
    b_s[2] = 0.0;
  }

  for (qp1q = 3; qp1q + 1 < 5; qp1q++) {
    qjj = (qp1q << 2) + 2;
    if (apply_transform) {
      xaxpy_xNufAhLU(2, -(xdotc_2UIAyAl6(2, b_A, 11, b_A, qjj + 1) / b_A[10]),
                     11, b_A, qjj + 1);
    }

    e[qp1q] = b_A[qjj];
  }

  for (qq = 2; qq + 1 < 5; qq++) {
    U[qq + 8] = b_A[qq + 8];
  }

  m = 2;
  b_s[3] = b_A[15];
  e[2] = b_A[14];
  e[3] = 0.0;
  U[12] = 0.0;
  U[13] = 0.0;
  U[14] = 0.0;
  U[15] = 1.0;
  for (qp1q = 2; qp1q >= 0; qp1q--) {
    qq = (qp1q << 2) + qp1q;
    if (b_s[qp1q] != 0.0) {
      for (qp1jj = qp1q + 1; qp1jj + 1 < 5; qp1jj++) {
        qjj = ((qp1jj << 2) + qp1q) + 1;
        xaxpy_xNufAhLU(4 - qp1q, -(xdotc_2UIAyAl6(4 - qp1q, U, qq + 1, U, qjj) /
          U[qq]), qq + 1, U, qjj);
      }

      for (qjj = qp1q; qjj + 1 < 5; qjj++) {
        U[qjj + (qp1q << 2)] = -U[(qp1q << 2) + qjj];
      }

      U[qq]++;
      for (qq = 0; qq < qp1q; qq++) {
        U[qq + (qp1q << 2)] = 0.0;
      }
    } else {
      U[qp1q << 2] = 0.0;
      U[(qp1q << 2) + 1] = 0.0;
      U[(qp1q << 2) + 2] = 0.0;
      U[(qp1q << 2) + 3] = 0.0;
      U[qq] = 1.0;
    }
  }

  for (qq = 3; qq >= 0; qq--) {
    if ((qq + 1 <= 2) && (e[qq] != 0.0)) {
      qp1q = ((qq << 2) + qq) + 2;
      for (qjj = qq + 1; qjj + 1 < 5; qjj++) {
        qp1jj = ((qjj << 2) + qq) + 2;
        xaxpy_xNufAhLU(3 - qq, -(xdotc_2UIAyAl6(3 - qq, V, qp1q, V, qp1jj) /
          V[qp1q - 1]), qp1q, V, qp1jj);
      }
    }

    V[qq << 2] = 0.0;
    V[(qq << 2) + 1] = 0.0;
    V[(qq << 2) + 2] = 0.0;
    V[(qq << 2) + 3] = 0.0;
    V[qq + (qq << 2)] = 1.0;
  }

  ztest = e[0];
  if (b_s[0] != 0.0) {
    rt = muDoubleScalarAbs(b_s[0]);
    nrm = b_s[0] / rt;
    b_s[0] = rt;
    ztest = e[0] / nrm;
    xscal_xJbqLS2l(nrm, U, 1);
  }

  if (ztest != 0.0) {
    rt = muDoubleScalarAbs(ztest);
    nrm = rt / ztest;
    ztest = rt;
    b_s[1] *= nrm;
    xscal_xJbqLS2l(nrm, V, 5);
  }

  e[0] = ztest;
  ztest = e[1];
  if (b_s[1] != 0.0) {
    rt = muDoubleScalarAbs(b_s[1]);
    nrm = b_s[1] / rt;
    b_s[1] = rt;
    ztest = e[1] / nrm;
    xscal_xJbqLS2l(nrm, U, 5);
  }

  if (ztest != 0.0) {
    rt = muDoubleScalarAbs(ztest);
    nrm = rt / ztest;
    ztest = rt;
    b_s[2] *= nrm;
    xscal_xJbqLS2l(nrm, V, 9);
  }

  e[1] = ztest;
  ztest = b_A[14];
  if (b_s[2] != 0.0) {
    rt = muDoubleScalarAbs(b_s[2]);
    nrm = b_s[2] / rt;
    b_s[2] = rt;
    ztest = b_A[14] / nrm;
    xscal_xJbqLS2l(nrm, U, 9);
  }

  if (ztest != 0.0) {
    rt = muDoubleScalarAbs(ztest);
    nrm = rt / ztest;
    ztest = rt;
    b_s[3] = b_A[15] * nrm;
    xscal_xJbqLS2l(nrm, V, 13);
  }

  e[2] = ztest;
  if (b_s[3] != 0.0) {
    rt = muDoubleScalarAbs(b_s[3]);
    nrm = b_s[3] / rt;
    b_s[3] = rt;
    xscal_xJbqLS2l(nrm, U, 13);
  }

  e[3] = 0.0;
  qq = 0;
  nrm = muDoubleScalarMax(muDoubleScalarMax(muDoubleScalarMax(muDoubleScalarMax
    (b_s[0], e[0]), muDoubleScalarMax(b_s[1], e[1])), muDoubleScalarMax(b_s[2],
    ztest)), muDoubleScalarMax(b_s[3], 0.0));
  while ((m + 2 > 0) && (qq < 75)) {
    qp1jj = m + 1;
    do {
      exitg1 = 0;
      qp1q = qp1jj;
      if (qp1jj == 0) {
        exitg1 = 1;
      } else {
        rt = muDoubleScalarAbs(e[qp1jj - 1]);
        if ((rt <= (muDoubleScalarAbs(b_s[qp1jj - 1]) + muDoubleScalarAbs
                    (b_s[qp1jj])) * 2.2204460492503131E-16) || ((rt <=
              1.0020841800044864E-292) || ((qq > 20) && (rt <=
               2.2204460492503131E-16 * nrm)))) {
          e[qp1jj - 1] = 0.0;
          exitg1 = 1;
        } else {
          qp1jj--;
        }
      }
    } while (exitg1 == 0);

    if (m + 1 == qp1jj) {
      qp1jj = 4;
    } else {
      qjj = m + 2;
      k_ii = m + 2;
      exitg2 = false;
      while ((!exitg2) && (k_ii >= qp1jj)) {
        qjj = k_ii;
        if (k_ii == qp1jj) {
          exitg2 = true;
        } else {
          rt = 0.0;
          if (k_ii < m + 2) {
            rt = muDoubleScalarAbs(e[k_ii - 1]);
          }

          if (k_ii > qp1jj + 1) {
            rt += muDoubleScalarAbs(e[k_ii - 2]);
          }

          ztest = muDoubleScalarAbs(b_s[k_ii - 1]);
          if ((ztest <= 2.2204460492503131E-16 * rt) || (ztest <=
               1.0020841800044864E-292)) {
            b_s[k_ii - 1] = 0.0;
            exitg2 = true;
          } else {
            k_ii--;
          }
        }
      }

      if (qjj == qp1jj) {
        qp1jj = 3;
      } else if (m + 2 == qjj) {
        qp1jj = 1;
      } else {
        qp1jj = 2;
        qp1q = qjj;
      }
    }

    switch (qp1jj) {
     case 1:
      rt = e[m];
      e[m] = 0.0;
      for (qjj = m; qjj + 1 >= qp1q + 1; qjj--) {
        xrotg_Aek4oMo2(&b_s[qjj], &rt, &ztest, &sqds);
        if (qjj + 1 > qp1q + 1) {
          rt = e[qjj - 1] * -sqds;
          e[qjj - 1] *= ztest;
        }

        xrot_8SOrvP1x(V, (qjj << 2) + 1, ((m + 1) << 2) + 1, ztest, sqds);
      }
      break;

     case 2:
      rt = e[qp1q - 1];
      e[qp1q - 1] = 0.0;
      for (qjj = qp1q; qjj < m + 2; qjj++) {
        xrotg_Aek4oMo2(&b_s[qjj], &rt, &ztest, &sqds);
        rt = -sqds * e[qjj];
        e[qjj] *= ztest;
        xrot_8SOrvP1x(U, (qjj << 2) + 1, ((qp1q - 1) << 2) + 1, ztest, sqds);
      }
      break;

     case 3:
      ztest = muDoubleScalarMax(muDoubleScalarMax(muDoubleScalarMax
        (muDoubleScalarMax(muDoubleScalarAbs(b_s[m + 1]), muDoubleScalarAbs
                           (b_s[m])), muDoubleScalarAbs(e[m])),
        muDoubleScalarAbs(b_s[qp1q])), muDoubleScalarAbs(e[qp1q]));
      rt = b_s[m + 1] / ztest;
      smm1 = b_s[m] / ztest;
      emm1 = e[m] / ztest;
      sqds = b_s[qp1q] / ztest;
      smm1 = ((smm1 + rt) * (smm1 - rt) + emm1 * emm1) / 2.0;
      emm1 *= rt;
      emm1 *= emm1;
      if ((smm1 != 0.0) || (emm1 != 0.0)) {
        shift = smm1 * smm1 + emm1;
        sqrt_tEibmDbk(&shift);
        if (smm1 < 0.0) {
          shift = -shift;
        }

        shift = emm1 / (smm1 + shift);
      } else {
        shift = 0.0;
      }

      rt = (sqds + rt) * (sqds - rt) + shift;
      ztest = e[qp1q] / ztest * sqds;
      for (qjj = qp1q + 1; qjj <= m + 1; qjj++) {
        xrotg_Aek4oMo2(&rt, &ztest, &sqds, &smm1);
        if (qjj > qp1q + 1) {
          e[qjj - 2] = rt;
        }

        rt = b_s[qjj - 1] * sqds + e[qjj - 1] * smm1;
        e[qjj - 1] = e[qjj - 1] * sqds - b_s[qjj - 1] * smm1;
        ztest = smm1 * b_s[qjj];
        b_s[qjj] *= sqds;
        xrot_8SOrvP1x(V, ((qjj - 1) << 2) + 1, (qjj << 2) + 1, sqds, smm1);
        xrotg_Aek4oMo2(&rt, &ztest, &sqds, &smm1);
        b_s[qjj - 1] = rt;
        rt = e[qjj - 1] * sqds + smm1 * b_s[qjj];
        b_s[qjj] = e[qjj - 1] * -smm1 + sqds * b_s[qjj];
        ztest = smm1 * e[qjj];
        e[qjj] *= sqds;
        xrot_8SOrvP1x(U, ((qjj - 1) << 2) + 1, (qjj << 2) + 1, sqds, smm1);
      }

      e[m] = rt;
      qq++;
      break;

     default:
      if (b_s[qp1q] < 0.0) {
        b_s[qp1q] = -b_s[qp1q];
        xscal_xJbqLS2l(-1.0, V, (qp1q << 2) + 1);
      }

      qq = qp1q + 1;
      while ((qp1q + 1 < 4) && (b_s[qp1q] < b_s[qq])) {
        rt = b_s[qp1q];
        b_s[qp1q] = b_s[qq];
        b_s[qq] = rt;
        xswap_L2OVFn32(V, (qp1q << 2) + 1, ((qp1q + 1) << 2) + 1);
        xswap_L2OVFn32(U, (qp1q << 2) + 1, ((qp1q + 1) << 2) + 1);
        qp1q = qq;
        qq++;
      }

      qq = 0;
      m--;
      break;
    }
  }

  s[0] = b_s[0];
  s[1] = b_s[1];
  s[2] = b_s[2];
  s[3] = b_s[3];
}
