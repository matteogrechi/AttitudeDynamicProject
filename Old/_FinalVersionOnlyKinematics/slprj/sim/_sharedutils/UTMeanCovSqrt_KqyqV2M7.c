...
void UTMeanCovSqrt_KqyqV2M7(real_T Y1[2], real_T Y2[16], const real_T X1[4],
  real_T X2[32], real_T Ymean[2], real_T Sy[4], real_T Pxy[8])
{
  __m128d tmp_p;
  real_T a__1[16];
  real_T tmp[16];
  ...
  Ymean[1] = Y1[1];
  for (kk = 0; kk < 8; kk++) {
    Ymean[0] += Y2[kk << 1] * -0.125000125000125;
    Ymean[1] += Y2[(kk << 1) + 1] * -0.125000125000125;
  }
  ...
  for (kk = 0; kk < 2; kk++) {
    for (iAcol = 0; iAcol < 8; iAcol++) {
      tmp[iAcol + (kk << 3)] = Y2[(iAcol << 1) + kk] * 0.35355356737010163;
    }
  }

  qr_D8DARVyI(tmp, a__1, Sy);
  Sy[1] = 0.0;
  ...
  if (guard1) {
    for (kk = 0; kk < 2; kk++) {
      ...
      Ss[kk] += Sy[kk << 1] * Sy[0];
      Ss[kk] += Sy[(kk << 1) + 1] * 0.0;
      Y1_p[kk << 1] = Y1[0] * Y1[kk];
      Ss[kk + 2] = 0.0;
      Ss[kk + 2] += Sy[kk << 1] * Sy[2];
      Ss[kk + 2] += Sy[(kk << 1) + 1] * Sy[3];
      Y1_p[(kk << 1) + 1] = Y1_e * Y1[kk];
    }
    ...
    for (kk = 0; kk < 2; kk++) {
      Sy[kk] = 0.0;
      Sy[kk] += Ss[kk << 1] * Y1_p[0];
      Sy[kk] += Ss[(kk << 1) + 1] * Y1_p[2];
      Sy[kk + 2] = 0.0;
      Sy[kk + 2] += Ss[kk << 1] * Y1_p[1];
      Sy[kk + 2] += Ss[(kk << 1) + 1] * Y1_p[3];
    }
  ...
  for (kk = 0; kk < 4; kk++) {
    for (iAcol = 0; iAcol < 2; iAcol++) {
      X2_p[kk + (iAcol << 2)] = 0.0;
      for (i = 0; i < 8; i++) {
        X2_p[kk + (iAcol << 2)] += X2[(i << 2) + kk] * Y2[(i << 1) + iAcol];
      }
    }
  }

  for (kk = 0; kk <= 6; kk += 2) {
    tmp_p = _mm_loadu_pd(&X2_p[kk]);
