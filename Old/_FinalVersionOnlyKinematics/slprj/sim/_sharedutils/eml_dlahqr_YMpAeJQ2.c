...
#include "eml_dlahqr_YMpAeJQ2.h"

int32_T eml_dlahqr_YMpAeJQ2(real_T h[16], real_T z[16])
{
  real_T v[3];
  real_T a__3;
  real_T a__4;
  ...
  while ((!exitg1) && (i + 1 >= 1)) {
      ...
      if (k + 1 >= i) {
        ...
        for (b_k = m; b_k <= i; b_k++) {
          ...
          if (nr == 3) {
            ...
            for (nr = 0; nr < 4; nr++) {
              ba = (z[((b_k - 1) << 2) + nr] + z[(b_k << 2) + nr] * htmp1) + z
                [((b_k + 1) << 2) + nr] * aa;
              z[nr + ((b_k - 1) << 2)] -= ba * tst;
              z[nr + (b_k << 2)] -= ba * ab;
              z[nr + ((b_k + 1) << 2)] -= ba * h12;
            ...
            for (nr = 0; nr < 4; nr++) {
              ba = z[((b_k - 1) << 2) + nr] + z[(b_k << 2) + nr] * htmp1;
              z[nr + ((b_k - 1) << 2)] -= ba * tst;
              z[nr + (b_k << 2)] -= ba * ab;
            }
          }
    ...
    if (!goto150) {
      ...
      if ((i + 1 != L) && (L == i)) {
        ...
        xrot_8SOrvP1x(z, ((i - 1) << 2) + 1, (i << 2) + 1, cs, sn);
      }
