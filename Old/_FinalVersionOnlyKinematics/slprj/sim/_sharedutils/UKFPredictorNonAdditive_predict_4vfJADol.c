...
#include "attitudePropagation_MP18TdOF.h"
#include "UKFPredictorNonAdditive_predict_4vfJADol.h"

void UKFPredictorNonAdditive_predict_4vfJADol(const real_T Qs[36], real_T X1[7],
  real_T S[49], const real_T varargin_1[3])
{
  ...
  attitudePropagation_MP18TdOF(Y1, tmp, varargin_1);
  ...
  for (kkC = 0; kkC < 14; kkC++) {
    ...
    attitudePropagation_MP18TdOF(&Y2[7 * kkC], tmp, varargin_1);
  }
  ...
  for (kkC = 0; kkC < 12; kkC++) {
    ...
    attitudePropagation_MP18TdOF(&Y2[7 * (kkC + 14)], &X2noise[6 * kkC],
      varargin_1);
  }
