...
for j=1:T/dt
    if mod(j,jGyro)==0 || mod(j,jAngle)==0 % if the number is too low it diverges...
        %% State prediction
        n = numel(x);
        W0 = .25;
        Wj = (1 - W0)/(2*n+1);
        ...
        if mod(j,jAngle)==0
            ...
            % Applying correction
            x(1) = x(1) + K*(z - zf);
            P(1,1) = P(1,1) - K*Covzf*K';
        end
        %% State correction sensor 2
        if mod(j,jGyro)==0
            % Acquiring measurement
            % Problema con acquisiszione dati...
            z = h2(interp1(tReal,xReal,dt*j)') + 1e-2 * randn(1,1);
