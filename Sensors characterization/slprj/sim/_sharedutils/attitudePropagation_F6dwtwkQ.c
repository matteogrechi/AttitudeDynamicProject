...
void attitudePropagation_F6dwtwkQ(real_T x[7], const real_T u[3])
{
  real_T Omega[16];
  real_T Omega_p[16];
  real_T q_p[7];
  ...
  q[2] /= y;
  q[3] /= y;
  y = muDoubleScalarSign(q[0]);
  q_p[0] = y * q[0];
  q_p[1] = y * q[1];
  q_p[2] = y * q[2];
  q_p[3] = y * q[3];
  q_p[4] = (-0.665 * x[5] * x[6] + u[0]) / 1.009 * 0.01 + x[4];
  q_p[5] = (-0.092999999999999861 * x[6] * x[4] + u[1]) / 0.251 * 0.01 + x[5];
  q_p[6] = (0.7579999999999999 * x[4] * x[5] + u[2]) / 0.916 * 0.01 + x[6];
  for (k = 0; k < 7; k++) {
    x[k] = q_p[k];
  }
}
